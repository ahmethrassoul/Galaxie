...
TEST_CASE( "NAIVE vs MacOS METAL (v2)", "[macos-metal-v2]" )
{
    ...
    {
        if( (std::isfinite( sumErr_x ) == false) || (std::isnan( sumErr_x ) == true)  )
        {
            printf("> sumErr_x = %6d : %f\n", i, sumErr_x);
            exit(EXIT_FAILURE);
        }
        ...
        if( (std::isfinite( diff_x ) == false) || (std::isnan( diff_x ) == true)  )
        {
            printf("(x) value %d/%d\n", i, size);
            printf("- diff value %1.3f\n", diff_x);
            printf("- ref. value %1.3f\n", gr->pos_x[i]);
            printf("- mod. value %1.3f\n", gn->pos_x[i]);
        }
        if( std::isfinite( diff_y ) == false )
        {
            printf("(y) value %d/%d\n", i, size);
            printf("- diff value %1.3f\n", diff_y);
            printf("- ref. value %1.3f\n", gr->pos_y[i]);
    ...
    printf("[metal_v2 ] Max. errors (%1.5f, %1.5f, %1.5f) | Avg. errors (%1.5f, %1.5f, %1.5f) | FPS %7.3f | Speed-up %7.3f |\n", maxErr_x, maxErr_y, maxErr_z, avgErr_x, avgErr_y, avgErr_z, fps_mod, speedup);
    printf("size = %d\n", size);
    printf("sumErr_x = %1.3f (%d)\n", sumErr_x, std::isnan( sumErr_x ));
    printf("sumErr_y = %1.3f (%d)\n", sumErr_y, std::isnan( sumErr_y ));
    printf("sumErr_z = %1.3f (%d)\n", sumErr_z, std::isnan( sumErr_z ));
