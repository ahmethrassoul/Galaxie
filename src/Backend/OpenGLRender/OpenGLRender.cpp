...
//
//
OpenGLRender::OpenGLRender(const int _width, const int _height, const std::string colorfile, const bool _mode_sdl) : Render(_width, _height, colorfile)
{
    mode_sdl = _mode_sdl;
    ...
    const char* police = "./bin/DroidSans.ttf";
...
//
//
void OpenGLRender::setPointSize (const float value)
{
    pointSize = value;
}
//
...
//
//
void OpenGLRender::setSpriteSize(const float value)
{
    spriteSize = value;
}
//
...
//
//
void OpenGLRender::setGridEnable(const bool enable)
{
    enableGrid = enable;
}
//
...
//
//
void OpenGLRender::setAxesEnable(const bool enable)
{
    enableAxes = enable;
}
//
...
//
//
void OpenGLRender::refresh_interface()
{
    if( mode_sdl == false )
        return;
    ...
    while ( SDL_PollEvent( &event ) )
    {
        ...
        if ( e == SDL_MOUSEMOTION ) {
            ...
            } else if ( (event.key.keysym.sym == SDLK_PLUS) || (event.key.keysym.sym == 'a') ) {
            ...
            } else if ( (event.key.keysym.sym == SDLK_MINUS) || (event.key.keysym.sym == 'q') ) {
                pointSize -= scaleFactor * 0.02f;
                LIMIT(pointSize, 1.0f, scaleFactor * 2.0f);
...
//
//
void OpenGLRender::refresh_fps(const float avgFPS)
{
    //
    ////////////////////////////////////////////////////////////////////////////////
    //
